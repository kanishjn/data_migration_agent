...
import { mockReasoningOutputs, mockPatterns } from '@/lib/mock-data';
import { formatRelativeTime } from '@/lib/utils';
import { cn } from '@/lib/utils';

export default function ReasoningPage() {
  const [expandedId, setExpandedId] = useState<string | null>(mockReasoningOutputs[0]?.id || null);

  // Get the linked pattern for a reasoning output
  const getLinkedPattern = (patternId: string) => {
  ...
  return (
    <MainContent>
      ...
      <StaggerContainer className="space-y-4">
        {mockReasoningOutputs.map((reasoning) => {
              ...
              >
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-zinc-600 dark:text-zinc-400 line-clamp-2">
                        {reasoning.primary_hypothesis.explanation}
                      </p>
                  ...
                  <div className="flex items-center gap-3">
                    <ConfidenceMeter value={reasoning.primary_hypothesis.confidence} size="sm" />
                    {isExpanded ? (
                    ...
                    >
                      <div className="p-5 space-y-6">
                        ...
                        <div className="p-4 rounded-lg bg-amber-500/5 border border-amber-500/20">
                          ...
                          <div className="flex items-center gap-3">
                            <ConfidenceMeter value={reasoning.primary_hypothesis.confidence} size="md" />
                          </div>
                        </div>
                        ...
                        {reasoning.alternative_hypotheses.length > 0 && (
                          <div>
                            <h4 className="text-sm font-medium text-zinc-900 dark:text-white mb-3 flex items-center gap-2">
                            ...
                            <div className="space-y-2">
                              {reasoning.alternative_hypotheses.map((alt, index) => (
                                <div
                                  key={index}
                                  className="p-3 rounded-lg surface-subcard surface-border"
                                >
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-zinc-500">{alt.why_less_likely}</span>
                                  </div>
                                </div>
                              ))}
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              {reasoning.unknowns.map((unknown, index) => (
                                <div
                                  key={index}
                                  className="flex items-start gap-2 p-2 rounded bg-fuchsia-500/5 border border-fuchsia-500/10"
                                >
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              {reasoning.assumptions.map((assumption, index) => (
                                <div
                                  key={index}
                                  className="flex items-start gap-2 p-2 rounded bg-amber-500/5 border border-amber-500/10"
                                >
                        ...
                        <div className="pt-4 border-t border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
                          <div className="flex items-center gap-2 text-xs text-zinc-500">
                            ...
                            <code className="px-2 py-1 rounded surface-subcard font-mono">
                              {reasoning.id}
                            </code>
                          </div>
                          <Link
                            href={`/decisions?reasoning=${reasoning.id}`}
      ...
      {mockReasoningOutputs.length === 0 && (
        <ScrollReveal>
          <div className="rounded-lg surface-card surface-border p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-zinc-900 dark:text-white mb-2">No reasoning outputs yet</h3>
            <p className="text-sm text-zinc-500 max-w-md mx-auto">
              Once patterns are detected, the LLM reasoning layer will generate hypotheses with structured analysis.
            </p>
          </div>
        </ScrollReveal>
      )}
      ...
      <ScrollReveal delay={0.2} className="mt-6">
        <div className="rounded-lg surface-card surface-border p-5">
          ...
          <div className="space-y-3">
            {mockReasoningOutputs.slice(0, 5).map((reasoning, index) => (
              <div
                key={reasoning.id}
                className="flex items-center justify-between p-3 rounded-lg hover-surface transition-colors"
              >
                <div className="flex items-center gap-3">
                  <div className={cn(
                    'w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium',
                    reasoning.primary_hypothesis.confidence >= 70
                      ? 'bg-emerald-500/10 text-emerald-500'
                      : reasoning.primary_hypothesis.confidence >= 50
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">
                      {formatRelativeTime(reasoning.created_at)} • {reasoning.primary_hypothesis.confidence}% confidence
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => setExpandedId(reasoning.id)}
                  className="text-xs text-cyan-600 dark:text-cyan-400 hover:underline"
                >
                  View
                </button>
